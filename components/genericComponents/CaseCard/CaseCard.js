...
export default function CaseCard({ blok, isQuote, mainContent, info, extraInfo, logo, order, tags, joinus, younexttext, slug, widthclass }) {
	let orderColor;
	switch (order % 4) {
		case 0:
			orderColor = "--green";
	...
	return (<>{
		isQuote ?
			...
			!joinus ? <div {...storyblokEditable(blok)} className={[css["case-card" + orderColor], css["case-card--" + widthclass]].join(" ")}>
				...
				{blok.blocktype != "one" && blok.cardimage && blok.cardimage.filename && <figure className={[css["case-card__cardimage-container"], css["case-card__cardimage-container--" + blok.blocktype]].join(" ")} style={backgroundImageStyle}></figure>}
